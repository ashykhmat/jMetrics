...
public class JMetricsApi {
     ...
     * 
     * @param pathProject - location of a project on a hard drive
     * @return - project content (names of classes, methods) and their metrics
     */
    public ProjectReport calculateMetrics(String pathProject) {
        try {
            return projectVisitor.visit(pathProject);
        } catch (VisitorException e) {
            LOGGER.error("Error during Maintainability Index metric calculation", e);
            return null;
        }
    }
     ...
     * 
     * @param pathToFile - location of a report file that will be used to write
     *            metrics
    ...
    public boolean writeMetricsToExcel(String pathToFile, ProjectReport project) {
        return excelWriter.writeMetricsToExcel(pathToFile, project);
    }
